...
class OldPauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		levelInfo.setFormat(Paths.font("VCRREAL.ttf"), 32);
		...
		blueballedTxt.setFormat(Paths.font('VCRREAL.ttf'), 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 101, 0, "PRACTICE MODE", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(Paths.font('VCRREAL.ttf'), 32);
		...
		chartingText.setFormat(Paths.font('VCRREAL.ttf'), 32);
	...
	function regenMenu():Void {
		...
		for (i in 0...menuItems.length) {
			...
			if(menuItems[i] == 'Skip Time')
			{
				...
				skipTimeText.setFormat(Paths.font("VCRREAL.ttf"), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
